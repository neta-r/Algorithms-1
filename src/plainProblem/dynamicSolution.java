...
public class dynamicSolution {
    ...
    public dynamicSolution(node[][] mat) {
        this.mat = mat;
        fillMatrix();
        traceBack();
    }
    ...
    public void traceBack() {
        ...
        for (int i = mat.length - 1, j = mat.length - 1; count < mat.length; ) {
            if (j - 1 >= 0 && mat[i][j].price == mat[i][j - 1].price + mat[i][j - 1].costRight) {
                ans.add(mat[i][j - 1]);
                j--;
                count++;
            } else if (i - 1 >= 0 && mat[i][j].price == mat[i - 1][j].price + mat[i - 1][j].costDown) {
                ans.add(mat[i - 1][j]);
                i--;
                count++;
            } else break;
    ...
    public static void main(String[] args) {
        ...
        mat[2][1] = h;
        mat[2][2] = i;
        dynamicSolution m = new dynamicSolution(mat);
        System.out.println("----");
        ...
        mat2[2][1] = eight;
        mat2[2][2] = nine;
        dynamicSolution n = new dynamicSolution(mat2);
    }
}
